...
namespace OFXSharp
{
    public class OFXDocumentParser
    {
        ...
        private OFXDocument Parse(string ofxString)
        {
            ...
            {
                //It is possible that the header is in the new file format, but the contents is still in the old.
                ofxString = FixNewXMLHeaderButOldSGMLFormat(ofxString);
                doc.Load(new StringReader(ofxString));
            }
        ...
        private string FixNewXMLHeaderButOldSGMLFormat(string file)
        {
            var reader = new SgmlReader();

            var newFile = RemoveHeader(file);
        ...
        private string ParseHeader(string file)
        {
            ...
            var header = file.Substring(0, file.IndexOf("<OFX>"))
               .Split(new[] { '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries);
            ...
            //Remove header
            return RemoveHeader(file);
        }

        private string RemoveHeader(string file)
        {
            return file.Substring(file.IndexOf("<OFX>") - 1);
        }
